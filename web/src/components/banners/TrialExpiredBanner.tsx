...
export function TrialExpiredBanner({ data }: { data: SubscriptionExpiredData }) {
  ...
  return (
    <div className="rounded-xl border border-red-200 bg-red-50 p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-3">
...
export function SubscriptionExpiringBanner({ daysRemaining }: { daysRemaining: number }) {
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-3">
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-amber-900 flex items-center gap-1.5">
          <Clock className="w-4 h-4 text-amber-600 shrink-0" />
          باقي على انتهاء اشتراكك
          <span className="inline-flex items-center justify-center bg-amber-200 text-amber-800 rounded-md px-1.5 py-0.5 font-bold text-xs tabular-nums">
            {daysRemaining}
          </span>
          يوم
        </p>
