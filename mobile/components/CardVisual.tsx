...
export function CardVisual({
  ...
  const STRIP_INNER_PAD = 12;
  const STAMP_GAP = 8;
  // 0.42 is roughly the aspect ratio of the Apple Wallet pass strip
  // and matches how the web CardVisual ends up rendering for the
  // common 2-row layouts. Tuned by eye on a 360-wide column.
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          {logo ? (
            <Image
              source={{ uri: logo }}
              // Height is fixed; width is computed from the natural
              // aspect ratio so square logos don't render inside a
              // fat letterbox.
...
const styles = StyleSheet.create({
  ...
  headerLeft: {
    // `gap: 10` gives the title just enough breathing room from the
    // logo edge without leaving the kind of wide alley that made the
    // logo look disconnected from the card name. `flex: 1` lets the
    // title still take up the remaining header space (for truncation
    // via numberOfLines) while `justifyContent: 'flex-start'` pins
    // both the logo and the title flush to the leading edge of the
    // card so the logo sits on the visual left regardless of the
    // app-level layout direction.
    flexDirection: 'row',
    alignItems: 'center',
